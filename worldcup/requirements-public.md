...
Navigation is split into three tiers, each with distinct visual weight so they don't read as interchangeable — a bar of tabs styled identically to another bar of tabs is confusing regardless of what they contain.

1. **Utility bar** (`<nav class="utility-bar">`, above the `<h1>`) — meta navigation: year links between all World Cup pages and `History` on the left; `Home` on the right. Small, muted text (`#948f81`), not the red segmented-button style. This is the least prominent nav on the page — it's about which page you're on, not what you're looking at within it. The current page's year (or `History`) is shown as plain bold text; other links use dark blue `#1A5276`. There is no Data Entry toggle, no mode switch, and no admin affordance of any kind on this site — see `requirements-admin.md` for where data entry actually happens.
2. **Primary tabs** (`.primary-tabs.view-toggle`, below the `<h1>`) — the segmented Match List / Groups / Knockout / Rankings toggle, in that left-to-right order. This is the single most visually dominant control on the page (larger font/padding than the utility bar and any nested sub-toggle), because switching between these views is the main thing a user does.
3. **Sub-toggles** (`.sub-toggle`, e.g. Rank/Scale) — controls nested inside a specific view, styled as plain gray pills rather than the red segmented style, and positioned inside that view's content area (e.g. `.rankings-panel`) rather than in its own top-level bar. This signals "a setting for what you're looking at," not "another place to navigate to."
...
**Status: placeholder.** The tab strip includes a **Groups** tab (`#groups` hash) alongside Match List, Knockout, and Rankings. It currently renders a "coming soon" placeholder only.
