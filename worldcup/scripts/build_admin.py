...
Run this after changing the admin page template in this file. Unlike
build.py, this script does not embed data as JS constants — the admin site
is always served over HTTP, so each page fetches worldcup/data/<year>.json
and worldcup/data/teams.json at runtime instead.
...
<table>
  ...
  <tbody id="games"><tr><td colspan="4" class="loading">Loading…</td></tr></tbody>
</table>
...
async function main() {{
  const [data, teams] = await Promise.all([
    fetch(`../worldcup/data/${{YEAR}}.json`).then(r => r.json()),
    fetch('../worldcup/data/teams.json').then(r => r.json()),
  ]);
  ...
  for (const game of games) {{
    ...
    tr.innerHTML = `
      ...
      <td><div class="flag-cell">${{flagIcon(flagByTeam[game.homeTeam], game.homeTeam)}}${{esc(game.homeTeam)}} <strong>${{hs}}</strong></div></td>
      <td><div class="flag-cell"><strong>${{as_}}</strong> ${{flagIcon(flagByTeam[game.awayTeam], game.awayTeam)}}${{esc(game.awayTeam)}}</div></td>
    `;
