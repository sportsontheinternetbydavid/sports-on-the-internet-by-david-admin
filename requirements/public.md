...
- **`Football`** — Levels 2 and 3 switch to the tournament content below:
  ...
  - **Level 3 — Section**: two equal segments, `History` and `Tournaments`. `History` links directly to `history.html`. `Tournaments` links to the current/latest tournament page (`2026.html` today) — from there, that page's own Level 2 (see *World Cup pages* below) is how you reach any other year.
