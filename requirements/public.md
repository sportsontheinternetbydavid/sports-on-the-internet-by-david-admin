...
- Each team is shown as its flag (an SVG image served from `../site/football/worldcup/flags/`, kept in sync with the source copy at `worldcup/data/flags/`) only — never a printed name, see `brand-guidelines.md` → *Flags, Not Names* — with the team's name shown on hover (e.g. via a tooltip).
