...
- **Level 1 — Sport**: four chips — `Sports!`, `Football`, `Hockey`, `Swimming`. `Sports!` and `Football` are both real, selectable items: a single-choice, client-side toggle (no page reload) that controls what Levels 2 and 3 show below, using the fly in/out transition described in `transitions.md` → *Transitions*. `Hockey` and `Swimming` render as disabled items — muted, no hover, not a link, the same treatment `Euros`/`Copa América`/`Gold Cup` get one level down (see *World Cup pages* below). There is no Basketball item.
...
- **`Football`** — Levels 2 and 3 switch to the tournament content below:
  ...
  - **Level 3 — Section**: two equal segments, `Tournaments` and `History`. `Tournaments` links to the current/latest tournament page (`2026.html` today) — from there, that page's own Level 2 (see *World Cup pages* below) is how you reach any other year. `History` links directly to `history.html`.
- **Transition:** board — nothing shared between the two states; `Sports!`'s Levels 2/3 and `Football`'s Levels 2/3 are two independent chip sets with no structure in common (Level 1 above is the one thing that never moves, in either state). Item — one chip in `Football`'s state; the whole `Sports!` signature is the one exception, moving as a single typographic composition rather than a set of chips (see `navigation.md` → *The homepage signature is exempt*). How — both sets stay permanently in the DOM, stacked and swapping in parallel (see `transitions.md` → *Transitions* → *Implementation note*); each chip flies the standard horizontal conveyor, out right/in left, individually staggered.
- **On load:** matches the sitewide default (see `transitions.md` → *Initial display*) — `Sports!` is already there in its resting position; it never flies in. `Football`'s chip set is already present in the DOM but parked off-screen at rest (not merely hidden), so its *first* activation still gets the normal Transition treatment above rather than needing special-cased handling.
...
The shared nav "sticky notes" design system (the four nav levels, chip characteristics) has its own doc — see `navigation.md`. The sitewide fly in/out transition rules (a general information-display rule, not nav-specific) have their own doc too — see `transitions.md`. Those docs are this site's shared vocabulary; every other feature section in this doc (*Homepage* above, *World Cup pages* below, and everything under them) still states its own **Transition:**/**On load:** lines per feature, per the hard requirement in `transitions.md` → *Transitions (fly in/out)*, but the shared rules those lines point back to live there, not here.
...
1. **Level 1 — Utility bar** (`<nav class="utility-bar view-toggle">`) — two small cross-page links, `Home` and `History`. `Home` links back to the homepage; `History` links to `history.html` — see *History page* below for why History isn't reached via the tournament-year list one level down. There is no Data Entry toggle, no mode switch, and no admin affordance of any kind on this site — see `admin.md` for where data entry actually happens. `Home` is the identical chip — same label, same destination, same slot — on every tournament page and on `history.html`, so per `transitions.md` → *The general rule*, it never flies on a link between any two of those pages (tournament ↔ tournament, tournament ↔ History); only `History`/`Tournaments`, whichever one actually differs, moves. It only flies like a normal chip on the one link that actually takes it away: clicking `Home` itself, to the homepage, which has no `Home` chip of its own to persist onto.
2. **Level 2 — Tournament list** (`.page-toggle.primary-tabs.view-toggle`) — a loosely-wrapping row of chip items, one per World Cup, using a short `WC YY` label — `WC 26`, `WC 22`, `WC 18`, `WC 14`, `WC 10`, `WC 06`, `WC 02`, `WC 98` — last two digits of the year, zero-padded, most recent first. This is the single most visually dominant level (larger font/padding than the others — see `navigation.md` → *General nav chip characteristics* → *Size*), since picking a tournament is the primary reason to be on this row of pages at all. The current page is shown the same way the current tab is shown one level down: the selected-chip treatment described in `navigation.md` → *General nav chip characteristics*, non-linking. There is no `WC 30` placeholder — a tournament gets a chip here once it actually has a page.
3. **Level 3 — Primary tabs** (`.detail-bar.view-toggle`) — the segmented Match List / Groups / Knockout / Rankings toggle, in that left-to-right order, at the same non-dominant sizing as Level 1. Switching tabs uses the fly in/out transition described in `transitions.md` → *Transitions* — the view content below flies out and the new view flies in. **Transition:** board — nothing shared between the outgoing/incoming view (a table, a bracket, a flag grid share no structure), so the outgoing view's board exits fully off-screen as one piece; item — each outgoing/incoming view's own rows/items, individually staggered alongside it (see that view's own *Transition* line for specifics); how — the standard horizontal conveyor, board out right/in left at board pace, items the same direction at item pace.
4. **View detail row** — detail controls for whichever of the four views is currently active, rendered directly beneath Level 3 at the same non-dominant sizing. Not one of the four numbered nav Levels (see `navigation.md`'s Level table) — Level 4 is reserved sitewide for table/grid column headers only (see below) — just page-specific content attached to whichever Level 3 tab is active. Every view has one, even Groups (which just holds a "coming soon" note — see *Group Stage*), so the nav is always the same shape no matter which tab you're on; only one view's row is populated/visible at a time, swapped by JS when you switch tabs. See each view's own section for what its row contains: *Confederation panel* (Match List), *Round toggles* (Knockout Bracket), *Show eliminated* / *True rank* toggles (Tournament ELO Rankings).
5. **On load:** matches the sitewide default (see `transitions.md` → *Initial display*) — whichever view is selected on arrival (Match List by default, or another view via URL hash, e.g. `#knockout`) is already there, fully in place, regardless of how it was reached; it never flies in. The other three views' content doesn't exist on the page at all until first selected, at which point they get the normal Transition treatment from item 3 above like any later switch.

Pages without the primary tabs (`history.html`) show only Levels 1–2, styled identically — chips look the same whether or not there's a level below them, since nothing about the chip treatment depends on stacking position. Level 4 still applies there (the F4/F8/F16 table's header row).
...
- Each World Cup page has Level 2 (the tournament list) to switch between World Cup pages, and Level 1 (`Home`, `History`) to leave the tournament pages entirely — see *Navigation* under *World Cup pages* above for the full nav structure.
...
**Status: placeholder.** The tab strip includes a **Groups** tab (`#groups` hash) alongside Match List, Knockout, and Rankings. Its own detail-control row (see *Navigation* under *World Cup pages*) holds a single muted, non-interactive note — "Group Stage view — coming soon." — centered in that row; there is no other content and nothing below it in the page body. The row is kept present (not blank/hidden) so the nav is always the same shape no matter which tab is active, and once this view is built out, the note is replaced with its own controls the same way every other view's detail row works.
...
**Transition:** none of its own yet — this view has no internal toggle, just the disabled placeholder note. It only appears/disappears as part of the Level 3 board-level switch (see *World Cup pages* → *Navigation*), same as every other tab.
...
The bracket's round sequence is determined by its **size** — the number of teams entering the knockout stage (32/16/8/4), set once per tournament in the admin site. Rounds halve down to the semifinals, followed by a **Final** round holding two games — the earlier-numbered of the two is the third-place game, the later-numbered is the final.
...
**Every game, in every round, is just its two participant rows — flag and score, flag and score. Nothing else.** No card, no border, no shadow, no date, no rotation, and no winner/loser treatment of any kind (no tint, no greyscale, no dimming) — the score alone already says who won, so a second visual cue on top of it is redundant chrome, not clarity. Flags are shown noticeably larger than anywhere else on the site (see *Flags* above) precisely because there's nothing else in the row competing for space or attention, and centered within the round's column (see *Round toggles* below for the column itself) rather than hung against one edge. A participant is either:
- A resolved team, shown as its flag only — never a printed name, see `brand-guidelines.md` → *Flags, Not Names*. Its score is a small sticky note (`#FAD7A0`, the same material as every other sticky note on the site — the W/D/L diff highlight, the flag-name hover label) stuck on the flag's own bottom-right corner, with its own small independent tilt (deterministic per game/side, not random-per-load, since this is part of the page's resting state rather than a hover-triggered ephemeral note). A sticky note always sits on top of whatever it's stuck to, including a neighboring game's flag if the two happen to sit close together — it never reads as tucked behind one. Hovering the flag shows the team's name via the sitewide sticky-note label (see *Flags* above), same as every other flag on the site.
- An unresolved slot, shown as a flag-shaped placeholder — same footprint as a real flag, muted the same way any other not-yet-available item on the site is (the homepage's `Hockey`/`Swimming`, the Groups placeholder note), with a plain "?" standing in for the missing sticker. The "Winner of Game N" / "Loser of Game N" / "TBD" text this used to show in-line isn't gone, it's just not printed directly on the poster anymore — it surfaces the same way a team's name does, via the sitewide sticky-note hover label (see *Flags* above), so hovering an unresolved slot names the game feeding it exactly like hovering a resolved flag names its team.

**The Final and third-place game are the one exception, and the exception is small: one label, nothing more.** No card, no white background, no date — those two games are still just their bare flag+score rows (or flag-placeholder rows, if not yet reached), exactly like every other game in the tree. The only thing added is a small paper tag reading "Final" or "3rd Place", laid diagonally across the seam between the two rows — its bottom-left corner over the top flag, its top-right corner over the bottom flag, leaning toward the left — the way a label actually dropped onto a stack of two photos would land across the join, not float above both of them on its own header row. Red paper, near-black Permanent Marker text (marker only comes in black, same rule as every chip — see `navigation.md` → *General nav chip characteristics* → *Color*), a hard offset shadow rather than a soft blur: every piece of this site is a physically assembled object (see `brand-guidelines.md` → *Materials*), so this is its own scrap of paper, not bare typeset text floating over the flags. This is the one place in the bracket where a real distinction (deciding the tournament vs. advancing within it) earns a real visual distinction, rather than dressing every game up equally regardless of what it means.

This placeholder behavior also applies to any knockout game appearing in the Match List before it's resolved — the flag cell shows italic placeholder text ("Winner of Game N" / "TBD") instead of a broken flag image; Match List is a table of text and numbers throughout, not a flag-only view, so the flag-placeholder treatment here doesn't extend there.

There is no connector line drawn into or out of any game — with every game reduced to just its rows, a line would be one more piece of chrome earning its keep only by convention, not by need; the tree structure already reads from each round's column position and the generous vertical gap between one game and the next, deliberately wide relative to a game's own internal row spacing, so the eye never mistakes where one game ends and the next begins.

#### Round toggles

The Knockout view's own detail-control row in the nav, directly beneath Level 3 (see *Navigation* under *World Cup pages*), has one button per round in the tournament's configured sequence (e.g. Round of 32 / Round of 16 / Quarterfinals / Semifinals / Final), labelled with that round's name. All rounds are shown by default.
...
**Transition:** board — nothing; a round's column has its own fixed horizontal slot (below) that must never shift sideways, so there's no shared structure for a "board" to even mean here; item — each game within the round being shown/hidden, not the column as a whole; how — vertical, the one exception to the sitewide horizontal conveyor (see `transitions.md` → *Transitions* → *Direction*): a game lifts up and off when hidden, or drops down into place when shown, each individually staggered.

**On load:** matches the sitewide default — all rounds are shown by default (see below), and every visible round's games are already there in their fixed slots; none of them drop in, regardless of whether Knockout is the view shown on arrival or reached directly via `#knockout`.

**Each round has a fixed horizontal slot, aligned to its own toggle button** directly above it — same left position, same width. Every round's column shares one uniform width — the widest round name's own natural width, computed across all rounds regardless of which are currently shown — rather than each column sizing to only its own label (a narrow "Final" column stretching no further than "Final" needs used to make the tree read as mismatched widths instead of one continuous grid); the toggle buttons themselves are widened to match for the same reason, so a column still lines up exactly under its own button even though neither is sized to its own content anymore. A column is positioned by its own place in the tournament's round sequence (first round always leftmost, Final always rightmost) — not by how many rounds currently happen to be shown. Showing or hiding a round never shifts any *other* round horizontally, and there's no board here to speak of (see `transitions.md` → *Transitions*) — a round's own games are the items that fly: each one lifts up and off individually (a small random stagger between games, not a single synchronized column-wide move) when its round is hidden, or drops down into its slot individually when shown, rather than the bracket reflowing to close/open a gap. The one thing that does move sideways-adjacent is vertical: each visible round's games are spaced down the shared column height (the tallest currently-visible round's height, same as before), so hiding the tallest round can change that shared height and reflow the remaining rounds' games vertically within their own unmoved columns.

- **Shrinking** removes one round at a time, and only from an end of the current range: clicking the leftmost or rightmost *shown* round hides it — that round's games each lift up and off individually, out of the bracket's frame.
- **Growing** can jump straight to any *hidden* round in one click: clicking a hidden round reveals it and every round between it and the current range in a single step (e.g. from a range of just "Semifinals", clicking "Round of 32" reveals Round of 32, Round of 16, and Quarterfinals all at once). Each newly-revealed round's games drop down into their fixed slot individually, one small stagger apart — a big jump reads as a flurry of games snapping into place in quick succession, not the whole column arriving at once.
...
All of this view's own controls — the **Rank**/**Scale** toggle, **Show eliminated**, **True rank**, and the debug toggle — live together in this view's own detail-control row in the nav, directly beneath Level 3 (see *Navigation* under *World Cup pages*), left-aligned in that order, rather than inside the rankings panel itself. The panel below them starts clean.
...
**Transition:** none — like Rank/Scale above, this toggle is excluded from the fly in/out treatment (see `transitions.md` → *Transitions* → *Scope*): nothing appears/disappears, existing flags just show/hide in place, so it's an instant update, not board/item/how.
...
- *Header row*: the row of gameset labels (Initial / MD1 / MD2 / … or Initial / Game 1 / … for 2026) is this view's Level 4 (see *Navigation* under *World Cup pages*) — red `#C0392B` background, near-black text, matching every other view's Level 4, sitting just below this view's own detail-control row above it.
...
**Transition:** none — a hover tooltip, not a navigation-driven appear/disappear, so it's outside the scope of `transitions.md` → *Transitions* entirely: instant show, a timed instant hide, never a fly.
...
**Transition:** none — a debounced hover dim/highlight, not a navigation-driven appear/disappear, so outside the scope of `transitions.md` → *Transitions*.
...
- A draw settled on penalties still shows **D** — penalty-shootout winners aren't tracked in the data at all (see `admin.md` → *Penalty shootout treatment*), so there's nothing to distinguish a shootout win from a regular draw here.
...
**Not one of the tournament-year list's own destinations, but reachable from every tournament page's Level 1.** This page doesn't appear in the Level 2 `WC YY` tournament list (see *Navigation* under *World Cup pages*) — History compares *across* tournaments, so it doesn't belong nested inside the per-year pages' own year switcher, which is about moving between individual tournaments. Instead it's reached via Level 1: the homepage's `Football` → Level 3 `History` item (see *Homepage* → *Navigation*), or directly from any tournament page's own Level 1 `History` chip. Level 1 here returns the favor with a `Tournaments` chip (below), landing on the current/latest year, same destination as the homepage's own `Tournaments` link.
...
- **Level 1 — Utility bar**: `Home` and `Tournaments`, the same two-item shape and non-dominant sizing as the tournament pages' own Level 1 (`Home`, `History`). Same chip styling as everywhere else (see `navigation.md` → *General nav chip characteristics*). `Home` links to the homepage; `Tournaments` links to the current/latest tournament page (`2026.html` today). `Home` is the same chip as every tournament page's own `Home`, so it never flies on this page's arrival from or departure to a tournament page — see *World Cup pages* → *Navigation* above for the full rule.
- **Level 2 — Round**: a real part of `.page-nav` now (not a standalone control with its own one-off CSS, which is what caused the fused-red-bar bug this replaces — see *Table structure* below) — a `primary-tabs` segmented choice, same dominant styling as any other Level 2 on the site: **Final 4**, **Final 8**, **Final 16**. Single-choice, not multi-select like the old three-checkbox version — exactly one is selected at a time. **Final 16 is the default** on load. Switching rounds uses the fly in/out transition from `transitions.md` → *Transitions*. **Transition:** board — the table itself: header row, year column/labels, every row's position, and the 16-column width, none of which ever change (see *Layout* below); item — each flag+ELO cell's **content**, not the row and not the table; how — the outgoing round's flag flies out to the right, the incoming round's flag for that same slot flies in from the left, individually staggered flag by flag at item pace, rather than the whole row (or the whole table) moving as one block. A row's year never moves — only what's placed in it does. This page has no Level 3.
...
**Transition:** none — a hover dim/highlight, not a navigation-driven appear/disappear, so outside the scope of `transitions.md` → *Transitions*.
...
**Transition:** none — the outline is drawn/removed instantly; it's a debug overlay, not content appearing/disappearing in the sense `transitions.md` → *Transitions* covers.
