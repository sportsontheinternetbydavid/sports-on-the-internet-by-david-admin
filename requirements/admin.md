...
Clicking a game box selects it (highlighted border) and expands an inline panel directly below that box, within its own round's column — the same pattern as the match list's row-click panel (see *Match list* → *Row click* above), not a single shared panel living elsewhere on the page. The round's other games shift down while a panel is open; that's an accepted trade for matching the one panel pattern this site already uses everywhere else, rather than a second, differently-shaped control just for this tab. Clicking the same box again collapses it; clicking a different box collapses the open one and expands the new one. Only one panel is open at a time. The panel has:

1. The game number and its round's label (e.g. "#89 — Round of 16").
2. A date field.
3. A Home slot: a mode selector (**Team** / **From game**), showing either a team-shorthand text input or a game-number + winner/loser selector, depending on the mode. The **From game** option (and the winner/loser selector) only appears on games where it's structurally possible:
   - It's omitted entirely on the entry round (there's nothing earlier to defer to — an entry-round matchup always comes from a concrete source, group standings or an already-played earlier round, never a deferred placeholder — see *Setting the bracket size* above).
   - The game-number input is labelled with the immediately preceding round's name (e.g. "From game (Round of 16)") — the only round a slot may defer to.
   - The winner/loser selector only offers **loser** on the last round (the third-place game) — every other round's advancement is always by winner.
4. An Away slot, same shape as Home.
5. "Generate command" / copyable command field / "Copy", producing a `worldcup/scripts/set_bracket_game.py` command (see *Scripts* below), plus a line of hint text restating the from-game round constraint.

Only the fields you touch end up in the generated command — this only ever changes what you explicitly set, same principle as the match list panel. The panel's own constraints (no From game on the entry round, no loser outside the last round) prevent the most common mistakes before a command is ever generated; anything they don't catch (a from-game number that doesn't exist, or already feeds a different slot) is caught by `set_bracket_game.py` itself with a specific error message before anything is written — see *Scripts* below.
...
- `worldcup/scripts/set_bracket_game.py YEAR GAME_NUMBER [--date YYYY-MM-DD] [--home SHORTHAND | --home-from GAME:{winner|loser}] [--away SHORTHAND | --away-from GAME:{winner|loser}]` edits a knockout game's date and/or participants — only the options passed are changed. `--home`/`--away` set a concrete team directly (once known) and clear any `*-from` reference on that side; `--home-from`/`--away-from` defer to an earlier game's winner or loser instead and clear the concrete team. Refuses to run on a game with no `round` (i.e. not a knockout-bracket game) — use `set_result.py` for regular results. `--date` must be `YYYY-MM-DD`. A `--home-from`/`--away-from` reference is checked before anything is written: the referenced game must exist and be in the immediately preceding round (never an earlier or later one), `loser` must only be used on the last round's third-place game, and no other slot may already defer to that same game+result — a bad reference exits with a specific error instead of silently writing bad structure. The same check runs in `build.py`'s validation, so it also catches a bad reference from any other source (a hand-edit, a future script) — see `knockout.check_from_ref`, shared by both rather than duplicated. Same push/`--no-push` behavior as `set_result.py`.
